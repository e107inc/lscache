...
class lscache_dashboard // include plugin-folder in the name.
{
	...
	function activity()
	{
		...
		if(!$sql->gen("SELECT lscache_id,lscache_date,lscache_folder FROM `#lscache` WHERE lscache_date BETWEEN ".$month_start." AND ".$month_end))
		{
			return false;
		}
		...
		while($row = $sql->fetch())
		{
			...
			$key = date('Y-n-j', $row['lscache_date']);

			switch($row['lscache_folder'])
			{
				case "Facebook":
					$amt[$key]['facebook'] += 1;
					break;
			...
			$dateName[$key] = date('jS', $row['lscache_date']);
		}
	...
	{
		$var[0]['icon'] 	= "<img src='".e_PLUGIN."lscache/images/lscache_16.png' alt='' />";
		$var[0]['title'] 	= "My Title";
		$var[0]['url']		= e_PLUGIN_ABS."lscache/lscache.php";
		$var[0]['total'] 	= 10;
